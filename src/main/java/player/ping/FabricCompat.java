...
public final class FabricCompat {
    ...
    public static boolean isOperator(CommandSourceStack source) {
        ...
        for (String opName : opNames) {
            if (opName != null && opName.equalsIgnoreCase(currentName)) {
                debugPermission(
                        source,
                        "isOperator-opnames-true",
                        "isOperator -> true (matched '%s' in getOpNames)",
                        currentName
                );
                return true;
            }
        ...
        debugPermission(
                source,
                "isOperator-opnames-false",
                "isOperator -> false (name '%s' not present in getOpNames, count=%d)",
                currentName,
                opNames.length
        );
        return false;
    ...
    private static boolean hasPermissionNode(CommandSourceStack source, String permissionNode) {
        try {
            ...
            Method checkMethod = permissionsClass.getMethod("check", CommandSourceStack.class, String.class, boolean.class);
            Object result = checkMethod.invoke(null, source, permissionNode, false);
            ...
            debugPermission(source, "permissions-node-nonboolean-" + permissionNode, "Permission node '%s' returned non-boolean result", permissionNode);
        } catch (ClassNotFoundException e) {
            debugPermission(source, "permissions-api-missing", "Fabric Permissions API not installed; skipping node checks");
        } catch (Exception e) {
