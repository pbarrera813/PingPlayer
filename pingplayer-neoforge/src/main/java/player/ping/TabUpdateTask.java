...
public final class TabUpdateTask {
    ...
    public static void onEndTick(MinecraftServer server) {
        if (!PingSettings.getInstance().getShowPingOnTab()) {
            NeoForgeCompat.debugThrottled("tab/tick", "tab-disabled", DEBUG_TAB_THROTTLE_MS, "Skipped tab update tick because showPingOnTab=false.");
            return;
        }

        List<ServerPlayer> players = server.getPlayerList().getPlayers();
        if (players.isEmpty()) {
            NeoForgeCompat.debugThrottled("tab/tick", "tab-empty", DEBUG_TAB_THROTTLE_MS, "Skipped tab update tick because there are no online players.");
            return;
        }
        ...
        NeoForgeCompat.debugThrottled("tab/tick", "tab-broadcast", DEBUG_TAB_THROTTLE_MS,
