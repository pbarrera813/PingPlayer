...
public final class IPCommand {
    ...
    private static int executeSelfIP(CommandContext<CommandSourceStack> context) {
        ...
        NeoForgeCompat.debug("command/ip", "Received /ip from '%s'.", source.getTextName());

        ServerPlayer player = source.getPlayer();
        if (player == null) {
            source.sendFailure(Component.literal("Console must use /ip <player>."));
            NeoForgeCompat.debug("command/ip", "Rejected /ip for non-player source '%s'.", source.getTextName());
            return 0;
        }
        ...
        if (address == null) {
            ...
            NeoForgeCompat.debug("command/ip", "Remote address was null for self lookup by '%s'.", source.getTextName());
            return 0;
        }
        ...
        NeoForgeCompat.debug("command/ip", "Resolved self IP for '%s'.", source.getTextName());
        return 1;
    }

    private static int executeTargetIP(CommandContext<CommandSourceStack> context) {
        CommandSourceStack source = context.getSource();
        NeoForgeCompat.debug("command/ip", "Received /ip <player> from '%s'.", source.getTextName());
        if (!NeoForgeCompat.hasAdminPermission(source)) {
            source.sendFailure(Component.literal("Only server admins can use /ip <player>."));
            NeoForgeCompat.debug("command/ip", "Denied /ip <player> for '%s' due to missing admin permission.", source.getTextName());
            return 0;
        }
        ...
        try {
            ...
            if (address == null) {
                source.sendFailure(Component.literal("Could not retrieve IP address for " + NeoForgeCompat.getProfileName(target)));
                NeoForgeCompat.debug("command/ip", "Remote address was null for target '%s'.", NeoForgeCompat.getProfileName(target));
                return 0;
            }

            String finalIp = normalizeIpAddress(address);
            Component message = Component.literal(NeoForgeCompat.getProfileName(target) + "'s public IP is: ")
                    .withStyle(style -> style.withColor(ChatFormatting.BLUE))
                    .append(Component.literal(finalIp)
                            .withStyle(style -> style.withColor(ChatFormatting.AQUA)));

            source.sendSuccess(() -> message, false);
            NeoForgeCompat.debug("command/ip", "Resolved IP for target '%s' via requester '%s'.", NeoForgeCompat.getProfileName(target), source.getTextName());
            return 1;
        } catch (Exception e) {
            source.sendFailure(Component.literal("Player not found or not online."));
            NeoForgeCompat.debug("command/ip", "Failed /ip for '%s': %s", source.getTextName(), e.getClass().getSimpleName());
            return 0;
        }
    }
    ...
    private static int executeHelp(CommandContext<CommandSourceStack> context) {
        CommandSourceStack source = context.getSource();
        NeoForgeCompat.debug("command/ip", "Displayed /ip help for '%s'.", source.getTextName());
